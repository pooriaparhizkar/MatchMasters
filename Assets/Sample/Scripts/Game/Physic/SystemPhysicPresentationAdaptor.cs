...
namespace Sample
{
    ...
    {
        public async void PlayPhysic(CellStack cellStack1, Action onCompleted)
        {
            ...
            if (cellStack1.HasTileStack())
            {
                ...
                presenter1.transform.DOMove(new Vector3(cellStack1.Position().x, cellStack1.Position().y - 1, 1), .5f);
