    ...
    public class NakamaManager : MonoBehaviour
    {
        ...
        public static NakamaManager Instance
        {
            get
            {
                lock (Lock)
                {
                    ...
                    if (go == null) go = new GameObject(SingletonName);

                    if (go.GetComponent<NakamaManager>() == null) go.AddComponent<NakamaManager>();
        ...
        private void Awake()
        {
            ...
            if (session == null || session.HasExpired(expiredDate))
            {
                ...
                {
                    if (t.IsCompleted) PlayerPrefs.SetString(SessionPrefName, t.Result.AuthToken);
